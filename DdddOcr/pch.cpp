...
#include <session/experimental_onnxruntime_cxx_api.h>
...
std::string oldlabelmap[] = { "", "掀", "袜", "顧", "徕", "榱", "荪", "浡", "其", "炎", "玉", "恩", "劣", "徽", "廉", "桂", "拂",
                                  "鳊", "撤",
...
void Init() {
    HMODULE ghmodule = GetModuleHandle(L"DdddOcr.dll");
    HRSRC hrsrc = FindResource(ghmodule, MAKEINTRESOURCE(101), L"ONNX");
    ...
    session = new Ort::Session(env, addr, size, session_options);

}


char* Classification(char* base64Img) {
    std::string b64 = base64Img;
    Mat image = Base2Mat(b64);

    Mat binary;

    int image_height = image.rows;
    int image_width = image.cols;
    Mat resizedData(int((64.0 / image_height) * image_width), 64, CV_8UC3);
    Size dsize = Size(int((64.0 / image_height) * image_width), 64);
    resize(image, resizedData, dsize, 0.5, 0.5, INTER_LINEAR);
    cvtColor(resizedData, binary, COLOR_BGR2GRAY);
    resizedData = binary;
    size_t input_tensor_size = 1 * 1 * 64 * resizedData.cols;
    size_t image_size = resizedData.rows * resizedData.cols;
    std::vector<float> input_tensor_values(input_tensor_size);
    int col = int((64.0 / image_height) * image_width);
    for (size_t pid = 0; pid < 64; pid++) {
        /** Iterate over all channels **/
        for (size_t ch = 0; ch < col; ch++) {
            input_tensor_values[pid * col + ch] = ((resizedData.ptr<uchar>(pid)[ch] / 255.0) - 0.5) / 0.5;
        }
    }
    ...
    std::vector<const char*> input_node_names_real = { "input1" };
    std::vector<const char*> output_node_names_real = { "output" };
    std::vector<int64_t> input_node_dims{ 1, 1, 64, resizedData.cols };
    ...
    int64* floatArray = output_tensors.front().GetTensorMutableData<int64>();

    std::vector<int64_t> outputShape = output_tensors[0].GetTensorTypeAndShapeInfo().GetShape();
    int64_t outputCount = std::accumulate(outputShape.begin(), outputShape.end(), 1, std::multiplies<int64_t>());
    std::vector<int64> outputData(floatArray, floatArray + outputCount);
    std::string result;
    int lastid = 0;
    for (size_t i = 0; i < outputData.size(); i++)
    {
        if (outputData[i] == lastid)
        {
            continue;
        }
        else
        ...
        if (outputData[i] != 0)
        {
            result += oldlabelmap[outputData[i]];
        }
    }
